...
class Category_model extends CI_Model {
    ...
    /*------------------------ get records  ------------------------- */
    public function getData($rowno,$rowperpage,$search = '',$_field = 'c.int_glcode',$_sort = 'asc')
    {          
    ...
    public function addRecord()
    {
        if($_FILES['var_image']['name'] != '')
        {
	        ...
	        $data = array(
		        'fk_parent' => $this->input->post('fk_parent'),
    ...
    {          
        $this->db->select('c.int_glcode,c.fk_parent,c.var_title,c.dt_createddate');
        $this->db->from('mst_category c');
        $this->db->where('c.chr_delete','N');      
        $result = $this->db->get();
        $row = $result->result_array();

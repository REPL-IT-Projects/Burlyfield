...
class News extends Admin_Controller {
	...
	public function index()
	{
		...
		$query = $this->db->where('chr_delete', 'N')->get('mst_news');
		$data['total_data'] = $query->num_rows();

		$data['data'] = $this->model->getData(0,10);

		// Pagination Configuration
		$config['base_url'] = base_url() . 'news/loadData/';
		...
		$this->load_view('news/view_news',$data);
	...
	/*------------------ edit view record ----------------------*/ 
	public function edit_news($userId)
	{
		$userId=base64_decode($userId);
		//echo $userId; exit();
		$data['data'] = $this->model->getIdByData($userId);
		$this->load_view('news/edit_news',$data);
	}

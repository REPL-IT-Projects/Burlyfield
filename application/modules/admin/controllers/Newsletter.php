...
class Newsletter extends Admin_Controller {
	...
	public function index()
	{
		...
		$query = $this->db->where('chr_delete','N')->get('mst_newsletter');
		$data['total_staff'] = $query->num_rows();

		$data['data'] = $this->model->getStaffData(0,10);

        // Pagination Configuration
		$config['base_url'] = base_url() . 'newsletter/loadStaff/';
		...
		$this->load_view('newsletter/view_newsletter',$data);
        ...
        public function delete_multiple() 
	{
        ...
        function submit_report(){
            ...
            $update = $this->db->update('mst_newsletter',$array);
        ...
        function export_csv(){
            ...
            $query = $this->db->query("SELECT * FROM mst_newsletter where chr_delete='N' ORDER BY int_glcode DESC");
            ...
            if(count($result) > 0){
                ...
                foreach($result as $key => $row){
                    $key += 1;
                    $lineData = array($key, $row['var_email'], $row['dt_createddate']);

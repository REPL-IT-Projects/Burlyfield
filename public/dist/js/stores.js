 ...
 $("#btn_search").click(function(){
  ...
  $.ajax({
    ...
    url: siteurl+'admin/stors/loadData/'+pagenumber,
    type: 'get',
  ...
  var entries = $('select[name=dp_entries]').val();
    ...
    function loadPagination(pagno,field = 'int_glcode',sort = 'desc',entries = '10'){
      ...
      $.ajax({
       ...
       url: siteurl+'admin/stors/loadData/'+pagno,
    ...
    function createTable(result,sno,total_rows,total_data){
       ...
       $('#stors_list tbody').empty();

       if(result == null || result == ''){

        var tr = '<tr><td colspan="6">No results</td></tr>';        

        $('#stors_list tbody').append(tr);
      ...
      for(index in result){
        ...
        var var_address = result[index].var_address;
        var var_lat = result[index].var_lat;
        var var_long = result[index].var_long; 
        ...
        tr += "<td><a href="+siteurl+"admin/stors/edit_stors/"+window.btoa(id)+"><i class=' fas fa-pencil-alt'></i> "+ var_name +"</a></td>";
        tr += "<td>"+ var_address +"</td>";
        tr += "<td>"+ var_lat +"</td>";
        tr += "<td>"+ var_long +"</td>";
        if(publish == 'Y'){

          var stors = "'stors'";
          var mst_module = "'mst_stors'"; 
          var chr_publish = "'chr_publish'";
          var N = "'N'";

          tr += '<td><a href="javascript:void(0);"><img id="tick-'+id+
          '" height="16" width="16" alt="" title="" src="'+siteurl+
          'public/assets/images/site_imges/tick.png" style="vertical-align:middle;cursor:pointer;" onclick="UpdatePublish('+stors+','+mst_module+','+chr_publish+','+N+','+id+');"></a></td>';            
        }else{
          ...
          tr += '<td><a href="javascript:void(0);"><img id="tick-'+id+
          '" height="16" width="16" alt="" title="" src="'+siteurl+
          'public/assets/images/site_imges/tick_cross.png" style="vertical-align:middle;cursor:pointer;" onclick="UpdatePublish('+stors+','+mst_module+','+chr_publish+','+Y+','+id+');"></a></td>';
        }   

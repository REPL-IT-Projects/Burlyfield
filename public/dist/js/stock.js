...
// var vendor = $("#fk_vendor").val();
  ...
  $.ajax({
    ...
    url: siteurl+'admin/stock/loadData/'+pagenumber,
    type: 'get',
...
// var vendor = $("#fk_vendor").val();
      ...
      $.ajax({
       ...
       url: siteurl+'admin/stock/loadData/'+pagno,
    ...
    function createTable(result,sno,total_rows,total_data){
       ...
       $('#stock_list tbody').empty();

       if(result == null || result == ''){

        var tr = '<tr><td colspan="6">No results</td></tr>';        

        $('#stock_list tbody').append(tr);
      ...
      for(index in result){
        ...
        var var_stock = result[index].var_stock;
        
        var stock_status = result[index].stock_status;
        ...
        if(publish == 'Y'){
          ...
          'public/assets/images/site_imges/tick.png" style="vertical-align:middle;cursor:pointer;" onclick="UpdatePublish('+stock+','+mst_module+','+stock_status+','+N+','+id+');"></a></td>';            
        }else{
          ...
          tr += '<td><a href="javascript:void(0);"><img id="tick-'+id+
          '" height="16" width="16" alt="" title="" src="'+siteurl+
          'public/assets/images/site_imges/tick_cross.png" style="vertical-align:middle;cursor:pointer;" onclick="UpdatePublish('+stock+','+mst_module+','+stock_status+','+Y+','+id+');"></a></td>';
        } 
        tr += '<td><button type="button" class="btn btn-secondary" data-toggle="modal" data-target="#stockEditModal"'+id+'" data-whatever="@mdo" data-id="'+id+'"><i class="fas fa-edit"></i></button></td>';  

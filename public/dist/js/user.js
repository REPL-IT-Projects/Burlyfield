 ...
 $("#btn_search").click(function(){
  ...
  $.ajax({
    ...
    url: siteurl+'admin/user/loadData/'+pagenumber,
    type: 'get',
  ...
  var entries = $('select[name=dp_entries]').val();
    ...
    function loadPagination(pagno,field = 'int_glcode',sort = 'desc',entries = '10'){
      ...
      $.ajax({
       ...
       url: siteurl+'admin/user/loadData/'+pagno,
    ...
    function createTable(result,sno,total_rows,total_data){
      ...
      for(index in result){
        ...
        if (var_image != '') {
          var image = siteurl+'uploads/user/'+var_image;
        } else {
        ...
        tr += "<td><a href="+siteurl+"admin/user/editUser/"+window.btoa(id)+"><i class=' fas fa-pencil-alt'></i> "+ var_name +"</a></td>";
        tr += "<td>"+ var_email +"</td>";
        tr += "<td>"+ var_mobile +"</td>";

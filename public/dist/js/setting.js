 ...
 $("#btn_search").click(function(){
  ...
  $.ajax({
    ...
    url: siteurl+'admin/setting/loadBanners/'+pagenumber,
    type: 'get',
    ...
    success: function(response){
      ...
      createTable(response.result,response.row,response.total_rows,response.total_banners);
    }
  });
});
  ...
  var entries = $('select[name=dp_entries]').val();
    ...
    function loadPagination(pagno,field = 'int_glcode',sort = 'desc',entries = '10'){
      ...
      $.ajax({
       ...
       url: siteurl+'admin/setting/loadBanners/'+pagno,
    ...
    function createTable(result,sno,total_rows,total_banners){
       ...
       if(result == null || result == ''){
        ...
        var tr = '<tr><td colspan="5">No results</td></tr>';        
      ...
      for(index in result){
        ...
        if (var_image != '') {
          var banner_img = siteurl+'public/assets/images/banners/'+var_image;
        } else {
          var banner_img = siteurl+'public/assets/images/site_imges/no_image.png';
        }
        ...
        tr += "<td><a href="+siteurl+"admin/setting/editBanner/"+window.btoa(id)+">"+ var_title +"</a></td>";
        tr += '<td><a class="example-image-link" href='+banner_img+
        ' data-lightbox="example-set" data-title="Click anywhere outside the image or the X to the right to close."><img class="example-image" src='+banner_img+
        ' id="cate_ig" alt='+banner_img+' /></a></td>';
        tr += "<td>"+dt_createddate+"</td>";
          if(publish == 'Y'){

          var setting = "'setting'";
          var mst_homepage = "'mst_homepage'"; 
          var chr_publish = "'chr_publish'";
          var N = "'N'";

          tr += '<td><a href="javascript:void(0);"><img id="tick-'+id+
          '" height="16" width="16" alt="" title="" src="'+siteurl+
          'public/assets/images/site_imges/tick.png" style="vertical-align:middle;cursor:pointer;" onclick="UpdatePublish('+setting+','+mst_homepage+','+chr_publish+','+N+','+id+');"></a></td>';            
        }else{
          ...
          tr += '<td><a href="javascript:void(0);"><img id="tick-'+id+
          '" height="16" width="16" alt="" title="" src="'+siteurl+
          'public/assets/images/site_imges/tick_cross.png" style="vertical-align:middle;cursor:pointer;" onclick="UpdatePublish('+setting+','+mst_homepage+','+chr_publish+','+Y+','+id+');"></a></td>';
        }   
        ...
        if(search == ''){
          ...
          $('#showing_').text('Showing '+start+' to '+end+' of '+total_rows+' entries (filtered from '+total_banners+' total entries)');

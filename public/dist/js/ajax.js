...
$('button[name=btn_banner_delete]').click(function ()
	{
	  // alert(window.location.href)
	  var modulename = $('#module').val();
	  var id = [];
	  ...
	  else
	  {
	  	if (confirm("Are you sure you want to delete records on this page?"))
	  	{
	  		$.ajax(
	  		{
	  			url: sitepath + "admin/"+modulename+"/delete_multiple_banner",
	  			method: 'POST',
	  			data:
	  			{
	  				id: id
...
$('button[name=btn_timeslot_delete]').click(function ()
	{
	  // alert(window.location.href)
	  var modulename = $('#module').val();
	  var id = [];
	  ...
	  else
	  {
	  	if (confirm("Are you sure you want to delete records on this page?"))
	  	{
	  		$.ajax(
	  		{
	  			url: sitepath + "admin/"+modulename+"/delete_multiple_timeslot",
	  			method: 'POST',
	  			data:
	  			{
	  				id: id
...
///////////////////// update publish /////////////////////////////////
	...
	function UpdatePublish(folder,tablename,fieldname,value,id){
		if (value == 'N') {
        var msg = "Do you want to really Unpublish ?";
    } else {
        var msg = "Do you want to really Publish ?";
    }
	swal({
...
///////////////////// update publish /////////////////////////////////
function UpdateBannerPublish(tablename,fieldname,value,id){
	if (value == 'N') {
        var msg = "Do you want to really Unpublish ?";
    } else {
        var msg = "Do you want to really Publish ?";
	...
	swal({
      ...
      if (result.value) {
		...
		dataType: 'json',
		cache: false,
		url: sitepath + "admin/settings/UpdateBannerPublish",
		data: { tablename: tablename,
			fieldname: fieldname,
			value: value,
			id: id },
			...
			success: function(data){
                ...
                $('#tick-'+id).attr('onclick' , "UpdateBannerPublish('"+tablename+"','"+fieldname+"','"+value1+"','"+id+"')");
            }
        });
            //$(location).attr('href','<?php //echo base_url() ?>album/deleteimges/'+id);
          }
